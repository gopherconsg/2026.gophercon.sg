...
- Sticky jump navigation in `workshops.astro`: responsive, scroll-aware design. A single `#workshop-nav` wrapper is `sticky top-[var(--header-height)]` with `backdrop-blur`. Pills always use grey text (#667085) on a semi-opaque white background (`rgba(255,255,255,0.85)`) for legibility against both the gradient and white page. A scroll listener adds `data-scrolled` once `scrollY > 80`, transitioning the nav to a fully opaque white background and setting pill background to solid white. Desktop always shows pills. Mobile shows all workshop pills initially so visitors see every option at a glance, then swaps to a compact `<select>` dropdown (with IntersectionObserver scroll-tracking) once scrolling begins. Display switching handled by CSS (`#workshop-nav[data-scrolled]` selectors in `<style is:inline>`) — no Tailwind class toggling in JS. Uses `overflow-x: clip` on `html, body` (not `hidden`) to avoid breaking sticky positioning. Native `scroll-behavior: smooth` on `html` for all anchor/scroll-to navigation site-wide
