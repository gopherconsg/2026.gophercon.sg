...
- Card-based layout for `/workshops` using Tailwind utility classes (individual workshop cards with header showing instructor photo/title/date, always-expanded sections for full curriculum and instructor bio, prerequisites callout, venue footer. Description split at first `####` heading — overview always visible, detailed syllabus in a separate section. Responsive scroll-aware sticky navigation below header (pills always show grey `#667085` text on `rgba(255,255,255,0.85)` background for legibility over gradient and white) — transparent at page top (blends with gradient), transitions to opaque white on scroll; desktop always shows pill links, mobile shows all pills initially then collapses to a scroll-tracking `<select>` dropdown. Instructor photos use `findSpeakerImage` with gradient initial fallback for missing images. Markdown content styles in Astro scoped `<style>` with `:global()`)
- Circular cropped speaker photos + keynote badge on `/speakers` page
- Previous year video embed option on home page hero
- "Become a Sponsor" section on home page with gradient background, brand-blue top border, descriptive copy, red CTA button linking to sponsorship info PDF (`public/docs/sponsorship-info.pdf`), and email contact link
...
- **Visual brand — best of both years:**
  ...
  - From 2019: Circular speaker photos, timeline layouts, keynote badge, location icon treatment, social icons in footer, "Become a Sponsor" section with PDF, previous year video embed.
...
- **Workshop cards (Tailwind utility classes):** Each workshop rendered as a standalone card in `WorkshopEntry.astro` using Tailwind utility classes — no custom CSS classes in `global.css`. Gradient header (instructor photo + title + date/time), body with overview text, always-expanded sections for curriculum and instructor bio, prerequisites callout, and venue footer. Description split at first `####` heading. Scroll-aware sticky jump navigation in `workshops.astro` (`sticky top-[var(--header-height)]`, `backdrop-blur`): starts transparent to blend with `.page-bg` gradient, transitions to `rgba(255,255,255,0.95)` via `data-scrolled` attribute when `scrollY > 80`. Desktop always shows pill links. Mobile shows all workshop pills initially (so visitors see all options at a glance), then swaps to a compact `<select>` dropdown with IntersectionObserver auto-tracking once scrolling begins. State-driven CSS in `<style is:inline>` with `#workshop-nav[data-scrolled]` selectors. Workshop cards use `scroll-mt-36` (9rem) to clear both sticky header and nav. Markdown content styling via Astro scoped `<style>` with `:global()` selectors. Mobile-responsive via Tailwind breakpoints (`flex-col md:flex-row`, etc.).
...
- **Component structure:** Header (responsive nav with per-item `enabled` flag + CTA button, mobile dropdown via `.navbar-nav` absolute positioning), Hero (with tagline, CTA button, optional video embed), Venue info (responsive heading that wraps on mobile), Speakers grid (CSS Grid, auto-fill/minmax), Tickets (Tito embed + `<noscript>` fallback with direct link to ti.to event page), Code of Conduct, Sponsors by tier with optional links; dedicated "Become a Sponsor" section shown when `!isArchived` (gradient bg `#f0fdff→#fff`, 4px brand-blue top border, descriptive copy, PDF CTA button linking to `public/docs/sponsorship-info.pdf`, email contact link — PDF URL in `content.toml` as `sponsors.sponsorshipPdfUrl`), Footer (social icons, copyright).
...
- **Smooth scrolling & overflow prevention:** `html { scroll-behavior: smooth; }` enables native CSS smooth scrolling for all anchor links and `scrollIntoView` calls site-wide. `html, body { overflow-x: clip; touch-action: manipulation; }` in `global.css`. Uses `clip` instead of `hidden` to avoid creating a scroll container that breaks `position: sticky`. `touch-action: manipulation` prevents double-tap zoom on mobile while preserving pinch-to-zoom. Header uses `height: auto; min-height: var(--header-height)` with `--header-height: 85px` defined in `@theme`. Consistent height across mobile and desktop — desktop nav items use the same padding as mobile so menu items don't push the header taller than the logo. Mobile nav dropdown centers CTA button via `text-align: center`. VenueInfo heading wraps on mobile (`overflow-wrap: break-word`), applies `white-space: nowrap` only at `min-width: 768px`.
