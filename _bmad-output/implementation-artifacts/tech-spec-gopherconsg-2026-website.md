...
- Card-based layout for `/workshops` (individual workshop cards with header showing instructor photo/title/date, collapsible `<details>` sections for full curriculum and instructor bio, prerequisites callout, venue footer. Description split at first `####` heading for progressive disclosure — overview always visible, detailed syllabus collapsed by default. Jump navigation pills at page top. Instructor photos use `findSpeakerImage` with gradient initial fallback for missing images)
...
- **Workshop cards (independent of Timeline):** Each workshop rendered as a standalone `.workshop-card` with gradient header (instructor photo + title + date/time), body with overview text, collapsible `<details>` for curriculum and instructor bio, prerequisites callout, and venue/additional info footer. Description is split at the first `####` heading for progressive disclosure. Jump navigation (`.workshop-nav`) at page top provides pill links to each card. Mobile-responsive: card header stacks vertically, instructor bio stacks vertically.
...
- **Sub-pages:** `/speakers` (detail card grid with circular photos, keynote badge, full bios, talk links, copy-link), `/schedule` (left-aligned timeline layout with Conference/Workshops tabs — tabs only shown when both nav items are enabled — active tab solid brand-blue, inactive white/grey), `/workshops` (card-based layout with jump navigation, collapsible details sections, matching Conference/Workshops tabs).
...
- **Overflow prevention:** `html, body { overflow-x: hidden; touch-action: manipulation; }` in `global.css`. `touch-action: manipulation` prevents double-tap zoom on mobile while preserving pinch-to-zoom. Header uses `height: auto; min-height: 5rem` to accommodate mobile nav expansion. Mobile nav dropdown centers CTA button via `text-align: center`. VenueInfo heading wraps on mobile (`overflow-wrap: break-word`), applies `white-space: nowrap` only at `min-width: 768px`.
