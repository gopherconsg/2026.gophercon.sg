...
- **Workshop cards (independent of Timeline):** Each workshop rendered as a standalone `.workshop-card` with gradient header (instructor photo + title + date/time), body with overview text, always-expanded sections (`.workshop-section`) for curriculum and instructor bio, prerequisites callout, and venue/additional info footer. Description is split at the first `####` heading. Sticky jump navigation (`.workshop-nav`, `position: sticky; top: 5rem; z-index: 40`) below header with frosted-glass background (`backdrop-filter: blur`). Workshop cards use `scroll-margin-top: 9rem` to clear both sticky header and nav. Mobile-responsive: card header stacks vertically, instructor bio stacks vertically, nav wraps and remains sticky.
...
- **Sub-pages:** `/speakers` (detail card grid with circular photos, keynote badge, full bios, talk links, copy-link), `/schedule` (left-aligned timeline layout with Conference/Workshops tabs — tabs only shown when both nav items are enabled — active tab solid brand-blue, inactive white/grey), `/workshops` (card-based layout with sticky jump navigation, always-expanded sections, matching Conference/Workshops tabs).
