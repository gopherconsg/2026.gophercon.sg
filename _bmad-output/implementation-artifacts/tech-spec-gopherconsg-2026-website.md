...
- **Workshop cards (Tailwind utility classes):** Each workshop rendered as a standalone card in `WorkshopEntry.astro` using Tailwind utility classes — no custom CSS classes in `global.css`. Gradient header (instructor photo + title + date/time), body with overview text, always-expanded sections for curriculum and instructor bio, prerequisites callout, and venue footer. Description split at first `####` heading. Responsive sticky jump navigation in `workshops.astro` — desktop: pill links (`hidden md:flex`, `sticky top-[var(--header-height)]`), mobile: compact `<select>` dropdown (`md:hidden`) with IntersectionObserver auto-tracking the visible workshop card. Workshop cards use `scroll-mt-36` (9rem) to clear both sticky header and nav. Markdown content styling via Astro scoped `<style>` with `:global()` selectors. Mobile-responsive via Tailwind breakpoints (`flex-col md:flex-row`, etc.).
