...
- **Overflow prevention:** `html, body { overflow-x: hidden; }` in `global.css`. Header uses `height: auto; min-height: 5rem` to accommodate mobile nav expansion. VenueInfo heading wraps on mobile (`overflow-wrap: break-word`), applies `white-space: nowrap` only at `min-width: 768px`.
